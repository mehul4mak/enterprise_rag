...
def main() -> int:
    parser = argparse.ArgumentParser(description="Conversational RAG over a PDF (LangGraph agent).")
    ...
    print(
        f"[setup] backend={CONFIG.backend} | provider={CONFIG.llm_provider} | model="
        f"{CONFIG.ollama_model if CONFIG.llm_provider == 'ollama' else '(see .env)'}"
    )
    print(f"[setup] indexing {args.pdf} ...")
    try:
        agent = build_agent(args.pdf, CONFIG)
    except (FileNotFoundError, ValueError) as e:
        print(f"[error] {e}", file=sys.stderr)
        return 1
    print(
        f"[setup] {agent.retriever.num_chunks} chunks indexed across "
        f"{len(set(c.page for c in agent.retriever.all_chunks()))} pages.\n"
    )
    debug = args.debug
    print(
        "Ask questions about the document. Type :debug to toggle retrieval view, :quit to exit.\n"
    )
